...
jQuery.fn.loadUploadContent = function (fileUploadNum) {
    ...
    $("#divUploadArea" + fileUploadNum).wrap("<form id='UploadForm" + fileUploadNum + "' action='../handler/WizardWuUpload.ashx' method='post' enctype='multipart/form-data'></form>");
    ...
    $("#spanDelImg" + fileUploadNum).live('click', function () {  //为删除按钮关联事件处理代码，这里用了live
        ...
        //向服务器发送删除请求
        $.post("../handler/WizardWuUpload.ashx?act=delimg", { NewNameDelete: picNewName, NameDelete: picName }, function (msg) {
            if (msg == "1") {
                files.html("删除成功");
                alert("删除成功");
                showimg.empty();
