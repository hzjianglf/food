...
public class IndexManager {
	...
	private static String INDEX_DIR = "/approot/lucene/indexmanger/luceneIndex";
	private static String DATA_DIR = "/approot/lucene/indexmanger/luceneData";
	...
	public static String xls2String(File file) {
		...
		try {
			...
			/*
			 * jxl.Workbook rwb = Workbook.getWorkbook(fis); Sheet[] sheet =
			 * rwb.getSheets(); for (int i = 0; i < sheet.length; i++) { Sheet
			 * rs = rwb.getSheet(i); for (int j = 0; j < rs.getRows(); j++) {
			 * Cell[] cells = rs.getRow(j); for (int k = 0; k < cells.length;
			 * k++) sb.append(cells[k].getContents()); } }
			 */
			fis.close();
			result += sb.toString();
		} catch (Exception e) {
